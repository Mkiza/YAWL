...
public class App extends ApplicationWithUIManager {
	...
	Map<Place, Integer> fireTransition(FlatAccess flatNet, Map<Place, Integer> marking1, Transition transition, Map<Arc, Boolean> selectedInArcs, Map<Arc, Boolean> selectedOutArcs) {
				...
				if (selectedInArcs.containsKey(ptArc) && selectedInArcs.get(ptArc)) {
					...
					if (source instanceof PlaceNode) {
						...
						if (source instanceof Place) {
							...
							if (ptArc.getType() != null && ptArcAnnotation.getText().getValue() == ArcTypes.RESET_VALUE) {
		...
		for (Node transition: flatNet.getTransitions()) {
			if (transition instanceof Transition) {
				if (enabled(flatNet, marking, (Transition) transition)) {
					...
					} else if(splitType == TransitionTypes.AND_VALUE) {
						for (Object arc : flatNet.getOut(transition)) {
							if (arc instanceof Arc) {
								Arc ptArc = (Arc) arc;
								SelectArc arcAnnotation = AnnoFactory.eINSTANCE.createSelectArc();
...
//	}
	...
	boolean enabled(FlatAccess flatNet, Map<Place, Integer> marking, Transition transition) {
		...
		if (transition.getJoinType() != null) {
			...
			if (joinType == TransitionTypes.AND_VALUE) {
				int count = 0;
				int reset = 0;
				int size = flatNet.getIn(transition).size();
				for (Object arc : flatNet.getIn(transition)) {
					if (arc instanceof Arc) {
						Arc ptArc = (Arc) arc;
						ArcType ptArcAnnotation = ptArc.getType();
						...
						if (source instanceof PlaceNode) {
							...
							if (source instanceof Place) {
								...
								if (ptArcAnnotation != null & ptArcAnnotation.getText().getValue() == ArcTypes.RESET_VALUE) {
									count++;
									reset++;
